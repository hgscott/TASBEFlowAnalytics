...
if(numel(desc.voltage) && numel(alt_desc.voltage))
    if desc.voltage ~= alt_desc.voltage
        TASBESession.warn('Channel:Description','In channel %s, descriptor ''%s'' expected to be %d, but is %d',...
...
if(numel(desc.filter) && numel(alt_desc.filter))
    if ~strcmp(desc.filter,alt_desc.filter)
        TASBESession.warn('Channel:Description','In channel %s, descriptor ''%s'' expected to be %s, but is %s',...
