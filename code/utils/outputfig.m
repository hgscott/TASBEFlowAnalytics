...
if TASBEConfig.get('testing.fakeFigureSaves'), 
    TASBESession.warn('TASBE:outputfig:TestMode','Test mode: not actually saving figure %s/%s',path,name);
    return;
end;

% If directory doesn't exist, try to create it
if ~isdir(path),
    TASBESession.warn('TASBE:outputfig:MakeDirectory','Directory does not exist, attempting to create it: %s',path);
    mkdir(path);
end
