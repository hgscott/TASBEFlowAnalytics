...
% fluorescence to plasmid count
 ...
 function PEM = PlasmidExpressionModel(cfp_data,CFP_af,CFP_noise,ERF_per_plasmid,drop_threshold,n_components)
        ...
        if nargin > 0
            ...
            if(sum(which)<10)
                TASBESession.warn('Model:CFPDistributionSeparation','Not enough data to model');
            ...
            if (numel(PEM.fp_dist.mu) == 1 || max(PEM.fp_dist.weight)>0.999) && n_components>1, % failure to separate -> duplicate
                TASBESession.warn('Model:CFPDistributionSeparation','Distribution could not be separated');
                ...
                TASBESession.warn('Model:CFPDistributionSeparation','Less components than expected: %i vs. %i',numel(PEM.fp_dist.mu),n_components);
