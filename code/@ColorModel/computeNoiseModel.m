...
for i=1:numel(CM.Channels)
    ...
    for j=1:numel(CM.ColorPairFiles)
        ...
        if(sum(which)>0)
            ...
            TASBESession.warn('Model:Noise','Color mapping file "%s" has no significant fluorescent subpopulations',cp{4});
            minset(n_files) = 0;
        end
    end
    
    if n_files == 0,
        TASBESession.warn('Model:Noise','Cannot compute noise model for %s: no color mapping files available',getName(CM.Channels{i}));
        continue;
    end
